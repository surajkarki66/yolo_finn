...
def yolo_img_train(opt):
    ...
    for video_name in tqdm(os.listdir(opt.img_train)):
        if 'MVI' in video_name:
            ...
            for i, img_name in enumerate(os.listdir(os.path.join(opt.img_train, video_name, 'img1'))):
                if ((i % stride) == 0) & ('.jpg' in img_name):
                    ori_path = os.path.join(opt.img_train, video_name, 'img1', img_name)
                    new_path = os.path.join('./images/train/', img_name)
...
def yolo_img_val(opt):
    ...
    for video_name in tqdm(os.listdir(opt.img_val)):
        if 'MVI' in video_name:
            stride = int(1.0 / opt.sample)
            for i, img_name in enumerate(os.listdir(os.path.join(opt.img_val, video_name, 'img1'))):
                if ((i % stride) == 0) & ('.jpg' in img_name):
                    ori_path = os.path.join(opt.img_val, video_name, 'img1', img_name)
                    new_path = os.path.join('./images/val/', img_name)
                    shutil.copyfile(ori_path, new_path)
...
def parse_opt():
    ...
    parser.add_argument('--img_train', type=str, default='/media/vision/storage1/Datasets/UA_DETRAC/sorted/train/', help='train images path')
    parser.add_argument('--img_val', type=str, default='/media/vision/storage1/Datasets/UA_DETRAC/sorted/test/', help='validation images path')
    parser.add_argument('--lbl_train', type=str, default='/media/vision/storage1/Datasets/UA_DETRAC/UA_DETRAC/good_labels/train/', help='train labels path')
    parser.add_argument('--lbl_val', type=str, default='/media/vision/storage1/Datasets/UA_DETRAC/UA_DETRAC/good_labels/test/', help='validation labels path')
